...
export const Fret: React.FC<FretProps> = React.memo(
  ({ number, fretCount, neckLength, neckWidth }) => {
...
export const getFretOffset = (number: number, neckLength: number, fretCount: number): number => {
  if (number === 0) {
    return SIDE_MARGIN;
  } else if (number < 0) {
    return -10;
